...
*/
	...
	protected class ContainerLauncher implements Runnable {
		...
		public String getLaunchCommand(Container container) throws IOException {
			...
			outer: for (BlockStatus current : blockList) {
				synchronized (current) {
					if (!current.isStarted()) {
						for (int i = 0; i <
								current.getLocation().getHosts().length; i++) {
							String currentHost =
									current.getLocation().getHosts()[i] + ":8042";
			...
			String offsetString =
			Long.toString(blockToProcess.getLocation().getOffset());
			String lengthString =
			Long.toString(blockToProcess.getLocation().getLength());
			LOG.info("Reading block starting at {} and length {}",
